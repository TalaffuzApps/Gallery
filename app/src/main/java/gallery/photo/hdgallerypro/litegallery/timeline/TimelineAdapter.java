...
public class TimelineAdapter extends ThemedAdapter<ViewHolder.TimelineViewHolder> {
    ...
    public ViewHolder.TimelineViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (viewType == TimelineItem.TYPE_HEADER) {
            return new ViewHolder.TimelineHeaderViewHolder(LayoutInflater.from(context).inflate(
                    R.layout.view_timeline_header,
                    parent,
                    false));

        } else return new ViewHolder.TimelineMediaViewHolder(LayoutInflater.from(context).inflate(
                R.layout.card_photo,
                parent,
                false),
    ...
    @Override
    public void onBindViewHolder(@NonNull ViewHolder.TimelineViewHolder viewHolder, int position) {
        super.onBindViewHolder(viewHolder, position);
        TimelineItem timelineItem = getItem(position);

        if (viewHolder instanceof ViewHolder.TimelineHeaderViewHolder) {
            ViewHolder.TimelineHeaderViewHolder headerViewHolder = (ViewHolder.TimelineHeaderViewHolder) viewHolder;
            headerViewHolder.bind((TimelineHeaderModel) timelineItem);

        } else if (viewHolder instanceof ViewHolder.TimelineMediaViewHolder) {
            ViewHolder.TimelineMediaViewHolder mediaHolder = (ViewHolder.TimelineMediaViewHolder) viewHolder;
